...
export default function MediaFilter({ media_type, updater }: { media_type: string, updater: (filter: Filter) => void }) {
  ...
  return (
    <form className="flex flex-col lg:flex-row gap-3 sm:gap-5 mt-3 sm:mt-4" onSubmit={onSubmit}>
      <div className="flex flex-col items-start justify-center sm:flex-row sm:justify-around gap-3 sm:gap-5">
      ...
      <div className="flex flex-col items-start h-full sm:flex-row sm:justify-around sm:items-center gap-3 sm:gap-5">
        {media_type === "animes" && (
          <div className="flex gap-2 justify-center items-center h-full">
            <label htmlFor="media_rating" className="text-sm font-semibold">Rating:</label>
            <select id="media_rating" name="rating" onChange={handleInputChange} className="bg-background max-w-72 px-2 py-1 border-b-2 border-accent cursor-pointer focus:outline-none">
        ...
        <div className="flex gap-2 justify-center items-center">
          <label htmlFor="media_sfw" className="text-sm font-semibold lg:py-1">Safe For Work:</label>
      ...
      <button type="submit" className="font-semibold w-full sm:w-auto rounded-lg bg-accent py-1 px-3 cursor-pointer hover:bg-secondary transition-colors">Search</button>
    </form>
  )
}
